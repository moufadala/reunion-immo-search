...
<style>
:root{{--bg:#faf8f4;--ink:#211f1d;--muted:#716b64;--card:#fff;--line:#e8e1d8;--ok:#067647;--warn:#b54708;--bad:#b42318;--accent:#ff385c}}*{{box-sizing:border-box}}body{{margin:0;background:var(--bg);font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;color:var(--ink)}}main{{max-width:1120px;margin:auto;padding:28px 18px 60px}}header{{display:flex;justify-content:space-between;gap:18px;align-items:flex-start;margin-bottom:22px}}h1{{font-size:clamp(30px,5vw,54px);line-height:.95;margin:0;letter-spacing:-2px}}.sub{{color:var(--muted);max-width:680px;line-height:1.45}}.actions{{display:flex;gap:8px;flex-wrap:wrap}}a,button{{border:1px solid var(--line);background:#fff;border-radius:999px;padding:10px 13px;color:inherit;text-decoration:none;font-weight:750;cursor:pointer}}.cards{{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin:18px 0}}.kpi{{background:#fff;border:1px solid var(--line);border-radius:18px;padding:14px}}.kpi b{{display:block;font-size:26px}}.grid{{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}}.source{{background:var(--card);border:1px solid var(--line);border-radius:22px;padding:16px;box-shadow:0 2px 8px rgba(0,0,0,.035)}}.source.high{{border-color:rgba(180,35,24,.35);background:#fff7f5}}.source.warning{{border-color:rgba(181,71,8,.30);background:#fffaf3}}.source.ok{{border-color:rgba(6,118,71,.20)}}.source>div:first-child{{display:flex;justify-content:space-between;gap:10px;align-items:start}}.source strong{{font-size:20px}}.source span,.source p,.source small{{color:var(--muted)}}.source p{{margin:.7rem 0}}.nums{{display:grid;grid-template-columns:auto 1fr auto 1fr;gap:6px;align-items:baseline;margin-top:12px}}.nums b{{font-size:24px}}.toolbar{{position:sticky;top:0;background:rgba(250,248,244,.92);backdrop-filter:blur(12px);padding:10px 0;z-index:2}}@media(max-width:760px){{header{{display:block}}.cards{{grid-template-columns:repeat(2,minmax(0,1fr))}}.grid{{grid-template-columns:1fr}}a,button{{min-height:44px}}}}
</style></head><body><main><header><div><h1>Santé sources immo</h1><p class="sub">Vue non destructive de la fraîcheur par source. Une source stale ne supprime rien: elle indique seulement qu'il faut investiguer le scraper avant de polir le produit.</p></div><div class="actions"><a href="/">Dashboard</a><a href="/source_health.json">JSON</a><a href="/changes.html">Changements</a></div></header><section class="cards"><div class="kpi"><b>{esc(payload.get('summary',{}).get('source_count'))}</b><span>sources</span></div><div class="kpi"><b>{esc(counts.get('fresh',0))}</b><span>fresh</span></div><div class="kpi"><b>{esc(counts.get('aging',0)+counts.get('stale',0))}</b><span>à surveiller</span></div><div class="kpi"><b>{esc(len(payload.get('summary',{}).get('stale_or_attention_critical',[])))}</b><span>critiques attention</span></div></section><div class="toolbar"><button data-filter="all">Tout</button><button data-filter="fresh">Fresh</button><button data-filter="aging">Aging</button><button data-filter="stale">Stale</button><button data-filter="not-tested">Non testées</button></div><section class="grid" id="grid">{''.join(rows)}</section><p class="sub">Généré: {esc(generated)} · smoke run: {esc(payload.get('latest_smoke_summary',{}).get('run_id') or 'n.c.')}</p></main><script>document.querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{{const f=b.dataset.filter;document.querySelectorAll('.source').forEach(c=>{{const st=c.dataset.status, smoke=c.textContent.includes('not-tested');c.style.display=(f==='all'||st===f||(f==='not-tested'&&smoke))?'block':'none'}})}})</script></body></html>"""
