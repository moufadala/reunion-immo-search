...
COMMUNE_ALIASES: dict[str, list[str]] = {
    'Saint-Denis': ['saint denis', 'st denis', 'st-denis', 'denis', 'sainte clotilde', 'ste clotilde', 'moufia', 'bellepierre', 'la bretagne', 'chaudron', 'camelia', 'camélias', 'montgaillard', 'vauban', 'barachois'],
    'Sainte-Marie': ['sainte marie', 'ste marie', 'ste-marie', 'duparc', 'beausejour', 'beauséjour', 'la convenance', 'grande montee', 'grande montée', 'riviere des pluies', 'rivière des pluies'],
...
function knownLocations(){
 const set=new Map();
 (typeof all!=='undefined'?all:[]).forEach(x=>{[x.city,x.district,x.region,x.location,x.location_intelligence?.district_best,x.location_intelligence?.precise_location_label].forEach(v=>{if(v){const n=norm2(v); if(n.length>2 && !set.has(n)) set.set(n,v);}})});
 Object.entries(SEARCH_ONTOLOGY.commune_aliases||{}).forEach(([label,aliases])=>{set.set(norm2(label),label); (aliases||[]).forEach(a=>set.set(norm2(a),label));});
 return [...set.entries()].sort((a,b)=>b[0].length-a[0].length);
}
...
function parseNatural(q){
 let raw=norm2(q).replace(/\bst\b/g,'saint').replace(/\bste\b/g,'sainte'); let work=' '+raw+' ';
 let tokens=[], rooms=[], maxPrice=null, minPrice=null, minSurface=null, typeHint='', chips=[], amenities=[], near=[];
 [...work.matchAll(/\bt\s*([1-9])\b/g)].forEach(m=>{rooms.push(Number(m[1])); addChip(chips,'rooms','T'+m[1],Number(m[1]));});
 if(/\bstudio\b/.test(work)){rooms.push(1); addChip(chips,'rooms','Studio',1)}
 const between=work.match(/(?:entre|de)\s*(\d{3,5})\s*(?:et|a|à|-)\s*(\d{3,5})/); if(between){minPrice=Number(between[1]); maxPrice=Number(between[2]); addChip(chips,'budget','Entre '+minPrice+' et '+maxPrice+' €',maxPrice);}
 const p=work.match(/(?:moins de|moins|sous|max|budget|loyer|jusqu a|jusqua|<=?)\s*(\d{2,5})/); if(p){maxPrice=Number(p[1]); addChip(chips,'budget','≤ '+maxPrice+' €',maxPrice);}
 if(!maxPrice){const nums=[...work.matchAll(/\b(\d{3,5})\s*(?:€|eur|euros)?\b/g)].map(m=>Number(m[1])).filter(n=>n>=300&&n<=10000); if(nums.length){maxPrice=Math.max(...nums); addChip(chips,'budget','≤ '+maxPrice+' €',maxPrice,false);}}
 const m=work.match(/(\d{1,3})\s*(?:m2|m²|metres|metre|m)\b/); if(m){minSurface=Number(m[1]); addChip(chips,'surface','≥ '+minSurface+' m²',minSurface);}
 ...
 for(const [n,label] of knownLocations()){ if(n.length>2 && work.includes(' '+n+' ')){ addChip(chips,'location',label,label); work=work.replaceAll(' '+n+' ',' '); } }
 [...raw.matchAll(/(?:proche|pres|près|vers|autour de|a proximite de|à proximité de)\s+([a-z0-9 ]{3,40})/g)].forEach(mm=>{const val=mm[1].trim().split(/\s+(?:moins|avec|sans|t\d|studio)\b/)[0].trim(); if(val){near.push(val); addChip(chips,'near','Proche '+val,val,false);}});
 for(const [alias,label] of amenityAliases()){ if(alias && work.includes(' '+alias+' ')){ amenities.push(label); addChip(chips,'amenity',label,label,false); work=work.replaceAll(' '+alias+' ',' '); } }
 const stop=new Set(['le','la','les','l','un','une','des','du','de','d','a','au','aux','en','sur','dans','avec','sans','et','ou','pour','moins','sous','max','budget','loyer','jusqu','jusqua','entre','proche','pres','près','vers','autour','m2','m','metres','metre','min','minimum','plus','piece','pieces','p','ch','chambre','chambres','appartement','appart','apt','maison','villa','euro','euros','eur']);
 tokens=work.split(/\s+/).filter(Boolean).filter(x=>!stop.has(x) && !/^t[1-9]$/.test(x) && !/^\d{1,5}$/.test(x));
 return {raw, tokens:[...new Set(tokens)], rooms:[...new Set(rooms)], maxPrice, minPrice, minSurface, typeHint, amenities:[...new Set(amenities)], near, chips};
}
function relScore(x,parsed){const hay=x._hay||''; let score=0; for(const t of (parsed.tokens||[])){if(hay.split(' ').includes(t)) score+=7; else if(hay.includes(t)) score+=3;} (parsed.amenities||[]).forEach(a=>{if((x.feature_tags||[]).includes(a)||hay.includes(norm2(a)))score+=12;}); (parsed.near||[]).forEach(n=>{if(hay.includes(norm2(n)))score+=9;}); return score;}
function reasons(x,parsed){const out=[]; const hay=x._hay||''; if(parsed.rooms?.length && parsed.rooms.includes(Number(x.rooms))) out.push('type demandé'); if(parsed.maxPrice && x.price&&x.price<=parsed.maxPrice) out.push('budget OK'); (parsed.amenities||[]).forEach(a=>{if((x.feature_tags||[]).includes(a)||hay.includes(norm2(a))) out.push(a);}); (parsed.near||[]).forEach(n=>{if(hay.includes(norm2(n))) out.push('proche '+n);}); if(parsed.tokens?.some(t=>hay.includes(t))) out.push('texte pertinent'); if((x.local_image_urls||[]).length>1) out.push('galerie'); return out.slice(0,5);}
function renderChips(parsed){const root=document.querySelector('#nlChips'); if(!root)return; root.innerHTML=(parsed.chips||[]).map((c,i)=>`<span class="nlchip ${c.strict?'':'soft'}" title="${c.strict?'Filtre strict':'Préférence / signal de ranking'}">${esc2(c.label)}<button type="button" data-chip-remove="${i}" aria-label="Retirer ${esc2(c.label)}">×</button></span>`).join(''); const txt=(parsed.chips||[]).map(c=>c.label).join(', '); const u=document.querySelector('#understood'); if(u) u.textContent=txt?('Compris : '+txt):'Tape une recherche naturelle : budget, ville, type, quartier, parking, meublé…'; const sug=document.querySelector('#suggestions'); if(sug) sug.innerHTML=(SEARCH_ONTOLOGY.examples||[]).slice(0,5).map(e=>`<button type="button" data-suggest="${esc2(e)}">${esc2(e)}</button>`).join('');}
...
function removeChip(i){const parsed=parseNatural(state.q); const c=(parsed.chips||[])[Number(i)]; if(!c)return; const value=norm2(String(c.value||c.label)); const label=norm2(String(c.label||'')); const words=String(state.q||'').split(/\s+/).filter(w=>!value.includes(norm2(w)) && !label.includes(norm2(w))); state.q=words.join(' ').replace(/\s+/g,' ').trim(); if(typeof syncInputs==='function')syncInputs(); enhancedApply();}
const originalRender = typeof render==='function' ? render : null;
const originalCard = typeof card==='function' ? card : null;
if(originalCard){ card=function(x){ let h=originalCard(x); return h.replace('<img loading="lazy"', '<img loading="lazy" decoding="async"').replace('<div class="actions">', `${(x.feature_tags||[]).slice(0,2).map(t=>`<span class="pill">${esc2(t)}</span>`).join('')}<div class="actions">`); }; }
function enhancedApply(updateUrl=true){
 const parsed=parseNatural(state.q); renderChips(parsed); let res=all.filter(x=>!hidden.has(x.id));
 ...
 if(parsed.typeHint && !state.type) res=res.filter(x=>norm2(x.type).includes(parsed.typeHint));
 if(state.city) res=res.filter(x=>x.city===state.city); if(state.type) res=res.filter(x=>x.type===state.type); if(state.region) res=res.filter(x=>x.region===state.region);
 const maxPrice=state.maxPrice||parsed.maxPrice; if(maxPrice) res=res.filter(x=>x.price && x.price<=Number(maxPrice));
 const minPrice=state.minPrice||parsed.minPrice; if(minPrice) res=res.filter(x=>x.price && x.price>=Number(minPrice));
 const minSurface=state.minSurface||parsed.minSurface; if(minSurface) res=res.filter(x=>x.surface && x.surface>=Number(minSurface));
 if(state.minRooms) res=res.filter(x=>x.rooms && Number(x.rooms)>=Number(state.minRooms)); if(state.minBedrooms) res=res.filter(x=>x.bedrooms && Number(x.bedrooms)>=Number(state.minBedrooms)); if(state.minScore) res=res.filter(x=>Number(x.opportunity_score||x.score||0)>=Number(state.minScore));
 if(state.zones){const zs=state.zones.split('|').map(norm2).filter(Boolean); if(zs.length) res=res.filter(x=>{const hay=norm2([x.city,x.district,x.location,x.title,x.description].join(' ')); return zs.some(z=>hay.includes(z));});}
 const sort=state.sort; res.sort((a,b)=>sort==='price_asc'?(a.price||9e9)-(b.price||9e9):sort==='price_desc'?(b.price||0)-(a.price||0):sort==='surface_desc'?(b.surface||0)-(a.surface||0):sort==='recent'?String(b.seen_last_at||'').localeCompare(String(a.seen_last_at||'')):(relScore(b,parsed)*100+(b.opportunity_score||b.score||0)+(b.geo_quality?.score||0)*10+(b.image_quality?.valid_local_count?5:0))-(relScore(a,parsed)*100+(a.opportunity_score||a.score||0)+(a.geo_quality?.score||0)*10+(a.image_quality?.valid_local_count?5:0)));
 if(originalRender) originalRender(res); else render(res); addResultBadges(parsed);
 const empty=document.querySelector('#empty'); if(empty && !res.length && !empty.querySelector('.emptyActions')) empty.insertAdjacentHTML('beforeend','<div class="emptyActions"><button id="relaxBudget" type="button">Élargir budget +100€</button><button id="clearNatural" type="button">Retirer recherche texte</button></div>');
 if(updateUrl && typeof syncUrl==='function') syncUrl(); return res;
}
apply = enhancedApply; parseQuery = parseNatural; relevance = (x,tokens)=>relScore(x, Array.isArray(tokens)?{tokens}:tokens);
ready(()=>{
 const q=document.querySelector('#q'); if(q) q.placeholder='Ex: T2 Saint-Denis moins 900€ meublé parking';
 document.querySelector('#nlChips')?.addEventListener('click',e=>{const b=e.target.closest('[data-chip-remove]'); if(b) removeChip(b.dataset.chipRemove);});
 document.querySelector('#suggestions')?.addEventListener('click',e=>{const b=e.target.closest('[data-suggest]'); if(!b)return; state.q=b.dataset.suggest; syncInputs(); enhancedApply();});
 document.addEventListener('click',e=>{if(e.target.id==='relaxBudget'){const p=parseNatural(state.q); const v=Number(state.maxPrice||p.maxPrice||0); if(v){state.maxPrice=String(v+100); syncInputs(); enhancedApply();}} if(e.target.id==='clearNatural'){state.q=''; syncInputs(); enhancedApply();}});
 setTimeout(()=>{try{enhancedApply(false)}catch(e){console.error('natural search v2 apply failed',e)}}, 350);
});
...
def enhance_json(app: Path) -> dict[str, Any]:
    ...
    ontology={'version':'natural_search_v2','generated_at':data.get('generated_at'),'commune_aliases':COMMUNE_ALIASES,'amenity_aliases':AMENITY_ALIASES,'examples':EXAMPLES,'contract':['chips dynamiques de confirmation','type/ville/budget stricts','équipements/proximité en ranking','géographie prudente']}
...
def enhance_html(app: Path) -> None:
    ...
    ontology={'commune_aliases':COMMUNE_ALIASES,'amenity_aliases':AMENITY_ALIASES,'examples':EXAMPLES}
