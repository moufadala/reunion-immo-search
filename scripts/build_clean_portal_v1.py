...
const PRESETS=[
 {id:'family_north_1200',label:'Famille Nord ≤1200€',state:{region:'Nord',maxPrice:'1200',minRooms:'3'},hint:'Nord, 3 pièces ou plus, budget max 1200€'},
 {id:'t2_t3_1000',label:'T2/T3 ≤1000€',state:{q:'T2 T3',maxPrice:'1000',minRooms:'2'},hint:'Appartements 2-3 pièces dans le budget'},
 {id:'two_beds',label:'2 chambres+',state:{minBedrooms:'2',maxPrice:'1300'},hint:'Au moins 2 chambres'},
 {id:'st_denis',label:'Saint-Denis',state:{q:'Saint-Denis',maxPrice:'1200'},hint:'Saint-Denis et quartiers'},
 {id:'ste_marie',label:'Sainte-Marie',state:{q:'Sainte-Marie',maxPrice:'1200'},hint:'Sainte-Marie et secteurs proches'},
 {id:'opportunity',label:'Opportunités 70+',state:{minScore:'70',sort:'recommended'},hint:'Score opportunité élevé'}
];
...
function syncInputs(){['q','city','type','maxPrice','minSurface','minRooms'].forEach(id=>{$('#'+id).value=state[id]||'';}); $('#sort').value=state.sort||'recommended'; document.querySelectorAll('.chip').forEach(c=>c.classList.toggle('active',(c.dataset.region||'')===(state.region||''))); document.querySelectorAll('[data-preset]').forEach(b=>{const p=PRESETS.find(x=>x.id===b.dataset.preset); const active=p&&Object.entries(p.state).every(([k,v])=>String(state[k]||'')===String(v)); b.classList.toggle('active',!!active);});}
function initPresets(){ $('#presets').innerHTML=PRESETS.map(p=>`<button class="chip preset" data-preset="${attr(p.id)}" title="${attr(p.hint)}">${esc(p.label)}</button>`).join(''); }
function applyPreset(id){const p=PRESETS.find(x=>x.id===id); if(!p)return; resetSearchState(); Object.assign(state,p.state); syncInputs(); apply();}
...
$('#presets').onclick=e=>{const b=e.target.closest('[data-preset]'); if(!b)return; applyPreset(b.dataset.preset);};
