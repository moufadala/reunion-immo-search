...
def main() -> int:
    ...
    require(daily, 'run_step public_delta_guard python3 "$PROJECT/scripts/audit_public_delta_guard.py" --baseline "$PROJECT/artifacts/app" --candidate "$CLEAN_STAGE"', "daily refresh", errors)
    require(daily, 'run_step promote_app_candidate python3 "$PROJECT/scripts/promote_app_candidate.py" --candidate "$CLEAN_STAGE"', "daily refresh", errors)
    require(daily, 'run_step rollback_app_drill python3 "$PROJECT/scripts/rollback_public_app.py" --backup "$BACKUP_APP"', "daily refresh", errors)
